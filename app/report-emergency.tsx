...
export default function ReportEmergency() {
  ...
  const mapTypeToBackend = (type: EmergencyType): string => {
    const typeMap: Record<EmergencyType, string> = {
      accident: "ACCIDENT",
      fire: "FIRE",
      medical: "MEDICAL",
      flood: "FLOOD",
  ...
  // Auto-generate title based on emergency type
  const generateTitle = (type: EmergencyType, customType?: string): string => {
    if (type === "other" && customType) {
      return `${customType} Emergency`;
    }
    const typeLabel =
      emergencyTypes.find((e) => e.type === type)?.label || "Emergency";
    return `${typeLabel} Emergency Alert`;
  };
  ...
  const uploadMediaFiles = async () => {
    ...
    // Add photos - all media files use 'files' field name
    ...
    // Add videos - all media files use 'files' field name
    for (let i = 0; i < videos.length; i++) {
      const video = videos[i];
      const filename = video.split("/").pop() || `video_${i}.mp4`;
      const match = /\.([\w]+)$/.exec(filename);
    ...
    // Add audio recordings - all media files use 'files' field name
    for (let i = 0; i < audioRecordings.length; i++) {
      const audio = audioRecordings[i];
      const filename = audio.uri.split("/").pop() || `audio_${i}.m4a`;
      const match = /\.([\w]+)$/.exec(filename);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Prepare emergency data matching backend requirements
      ...
      const hasMediaFiles =
        photos.length > 0 || videos.length > 0 || audioRecordings.length > 0;

      let response;

      if (hasMediaFiles) {
        // Create FormData for multipart upload with media files
        const formData = await uploadMediaFiles();

        // Add emergency data fields
        formData.append("type", mapTypeToBackend(selectedType));
        formData.append("title", autoGeneratedTitle);
        formData.append("description", description.trim());
        formData.append("address", location);
        formData.append("latitude", latitude.toString());
        formData.append("longitude", longitude.toString());

        response = await emergencyApi.create(formData);
      } else {
        // Send as JSON if no media files
        const emergencyData = {
        ...
        response = await emergencyApi.create(emergencyData);
      }
      ...
      if (response.success) {
        setAlert({
          ...
          buttonText: "View Emergency",
          onButtonPress: () => {
            setAlert({ ...alert, visible: false });
            // Navigate to SOS tab to see the created emergency
